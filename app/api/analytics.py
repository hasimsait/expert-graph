...
async def list_documents_endpoint(repo: GraphRepository = Depends(get_graph_repository)) -> Dict[str, Any]:
    """List all ingested SourceDocument / Chunk nodes currently available in active repository asynchronously."""
    docs = await get_all_documents(repo=repo)
    return {
        "documents": docs,
        "count": len(docs)
    }
...
async def document_implications_endpoint(
    ...
    implications = await get_document_implications(document_id, repo=repo)
    return {
...
async def concept_pagerank_endpoint(repo: GraphRepository = Depends(get_graph_repository)) -> Dict[str, Any]:
    """Expose PageRank centrality scores for CanonicalConcept nodes asynchronously."""
    top_concepts = await run_concept_pagerank(repo=repo)
    return {
