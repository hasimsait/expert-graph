...
def get_pending_queue(
    ...
    items = repo.get_pending_queue(limit)
...
def approve_edge(
    ...
    """Approve a pending edge in the active graph repository."""
    repo.update_edge_status(edge_id, "approved", body.user_id)
...
def reject_edge(
    ...
    """Reject a pending edge in the active graph repository."""
    repo.update_edge_status(edge_id, "rejected", body.user_id)
    return {"status": "rejected", "edge_id": edge_id, "rejected_by": body.user_id}

@router.get("/stats")
def get_dashboard_stats(repo: GraphRepository = Depends(get_graph_repository)):
    """Get count of pending, approved, and rejected facts."""
    return repo.get_stats()

@router.post("/reset")
def reset_database_and_queue(repo: GraphRepository = Depends(get_graph_repository)):
    """Wipe graph database and reset state."""
    repo.reset_graph()
